...
model = open("REFERENCE.md", "r").read()
text = open("Source/ToProcess01.txt", "r").read()
template = open("teiTemplate.xml", "r").read()

response = client.models.generate_content(
    model='gemini-2.5-flash', contents=f"Taking the TEI encoding template in {template}, take the text in {text} and do the TEI encoding of that text by strictly following the encoding reference provided in {model}. Write the resulting document in an XML-TEI file.")
with open("Structural Versions/theIndicator15.xml", "w", encoding="utf8") as outfile:
    outfile.write(response.text) #change name of file following the model => Structural Versions/theIndicator15.xml
...
for i in range(1,4): #number for last argument of range depends on number of letters in the 'text' doc plus 1 (no index 0).
    ...
    response = client.models.generate_content(
        model='gemini-2.5-flash', contents=f"Taking the TEI encoding template in {template}, take the text in {text} of the corresponding issue {i} in the provided order (the issues are divided by dashed lines) and do the TEI encoding of that text by strictly following the encoding reference provided in {model}. Write the resulting document in an XML-TEI file.")
